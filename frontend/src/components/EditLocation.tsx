...
function EditLocationModal(props: EditLocationModalProps): JSXElement {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    let res = await API.PATCH(`locations/${props.selectedLocation.id}`, location());
    ...
    <div>
      <div class="modal modal-open">
        <div class="modal-box">
          ...
          <input type="text" class="input input-bordered w-full max-w-xs my-2"
          ...
          <input type="text" class="input input-bordered w-full max-w-xs my-2"
            ...
            onInput={(e) => handleLocationLevelChange(parseInt(e.target.value, 10))} />
          <h5 class="font-bold text-lg">Note: ID's may not be changed. You must create a new location and delete the existing one.<br />
            Be aware that you will lose all data about the location if it is deleted (timestamps, etc) </h5>
